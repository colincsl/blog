...
At the broadest level there are two categories of inference: exact and approximate. Exact inference methods deterministically compute the best set of labels. Unfortunately exact inference is only possible in acyclic graphs like chains and trees. Approximate inference methods typically rely on some sort of sampling (either on the structure of the graph or on individual node values) and may result in different labelings between different trials or with a different number of samples. Note that there is a vast literature on different inference methods for working with different types of data and model definitions. 

In addition there are two separate subproblems. In one problem we want to compute the marginal distribution, meaning the likelihood that each node is labeled with each classnode. In the second problem we want to just compute the most likely label at each node. In message passing the first method is referred to as sum-product and the latter is max-product. The most important marginal method to be familiar with is Belief Propagation. This is the basis for exact inference in trees and is also used in approximate methods such as Loopy BP and Tree Reweighted Message Passing (TRW). In BP, the potentials for each label node are collected into beliefs which are passed between nodes recursively. In order to do this algorithm justice I will save a detailed explanation for another time.
